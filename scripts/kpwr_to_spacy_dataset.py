...
def non_empty_ner(record):
    return True if record["ner"] else False
...
def main():
    ...
    dataset = load_dataset("clarin-pl/kpwr-ner").filter(function=non_empty_ner)
    ...
    info = (
        ...
        "Records with empty 'ner' field were omitted.\n\n"
        f"Train docs: {train_len} ({train_len / total_docs:.2%})\n"
        f"Validation docs: {val_len} ({val_len / total_docs:.2%})\n"
        f"Test docs: {test_len} ({test_len / total_docs:.2%})\n"
    )
